...
{ 
    ...
    public int currentOrder; // 현재 누가 대통령인지 - playerOrder의 인덱스로 기록
    ...
    public void SendPickPosition() // 포지션 뽑기
    {
        int index = 0;
    ...
    public void ShowPickChancellorInfo(Action pickChancellor) // 수상 뽑으라고 안내
    {
        ...
        texts[0].text = "내각 구성"; 
        texts[1].text = "대통령"; 
        texts[2].text = PhotonNetwork.CurrentRoom.Players[playerOrder[currentOrder]].NickName; // TODO: currentOrder 증가할 때 0으로 초기화
        texts[3].text = $"다음 대통령은 {PhotonNetwork.CurrentRoom.Players[playerOrder[currentOrder+1]].NickName} 입니다."; 
    ...
    public void PickChanellorInfo() // 수상 뽑기
    {
        ...
        {
            roomNameText.text = $"{PhotonNetwork.CurrentRoom.Players[playerOrder[currentOrder]].NickName}가 수상을 선정 중입니다.";
        }
    }
    ...
    private IEnumerator GameStartIntro() // 게임 시작 후 신분 인트로
    {
        ...
        // 자신의 신분 카드 활성화
        // TODO: 게임 종료 후 다시 비활성화 해야됨
        myPosObj.gameObject.SetActive(true);

        if ((Position)PhotonNetwork.LocalPlayer.CustomProperties["position"] == Position.pacist) // 파시스트일 경우
        {
            int index = 0;
